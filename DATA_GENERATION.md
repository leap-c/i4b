...
- controllers and simulation loops from `i4b/simulator.py`

The companion notebook is `notebooks/DataGeneration.ipynb`.
The reusable Python helpers live in `i4b/data_generation.py`.
...
Use `i4b.disturbances.load_weather(latitude, longitude, altitude, year, tz, repo_filepath)`.
...
The helper `i4b.disturbances.get_int_gains(...)` scales the selected profile by building floor area.
...
Both controller outputs then pass through the building's heating-system parameters in `i4b/simulator.py`:
...
- **`T_offset` sets the level of the heating curve.** Most catalog values undersupply the current models: with `sfh_1984_1994_1_enev` (`T_offset = -5`) the room settles at ~16 degC, about 3 K below the comfort bound. `i4b.controller.heatcurve.heatcurve.tune_building_t_offset_and_mdot_hp` re-tunes it (it suggests `+1` K for that building, which gives ~20 degC).
...
from i4b import data_generation as dg
