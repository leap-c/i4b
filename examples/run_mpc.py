...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Timestep in seconds (e.g., 900 for 15 minutes, 3600 for 1 hour)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--building", type=str,
        default="sfh_2016_now_0_soc",
        help="Building name (e.g., 'sfh_2016_now_0_soc', 'sfh_1958_1968_0_soc', 'i4c')"
    )
    parser.add_argument(
...
def select_building(building_name: str):
    """Load building parameters from i4b.data.buildings module.
...
def main() -> None:
    ...
    h = delta_t  # Internal variable for MPC (keep 'h' for MPC compatibility)
    ...
    int_gains_df = disturbances.get_int_gains(
        ...
        profile_path='i4b/data/profiles/InternalGains/ResidentialDetached.csv',
    ...
    if args.grid_on:
        grid_df = pd.read_csv(
            'i4b/data/grid/grid_signals.csv',
            sep=',',
            header='infer'
        )
