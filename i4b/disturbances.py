...
def generate_disturbances_all(building_model, year=2015, timestep=900, offset_days=0, GRID_ON=True, repo_filepath=''):
   ...
   int_gains_df = get_int_gains(time = weather_df.index,
                                   profile_path = data_path('profiles', 'InternalGains', 'ResidentialDetached.csv', repo_filepath=repo_filepath),
   ...
   # read grid data
   if GRID_ON: 
      data_grid = pd.read_csv(data_path('grid', 'grid_signals.csv', repo_filepath=repo_filepath),sep=',',header='infer')['EEX2015'].values*0.001*100 # Cent/kWh
...
def generate_disturbances(building, year=2015, repo_filepath=''):
    ...
    profile_path = data_path('profiles', 'InternalGains', f'{building.usage}.csv', repo_filepath=repo_filepath)
    int_gains_df = get_int_gains(time = weather_df.index, 
...
def load_weather(latitude, longitude, altitude=0, year=2015, tz='Europe/Berlin', repo_filepath=''):
    ...
    An example DWD weather file for a location close to Freiburg can be found in the i4b/data/weather/ directory.
    ...
    dwd_filepath = data_path('weather', dwd_filename, repo_filepath=repo_filepath)
...
def _fetch_pvgis_data(latitude, longitude, two_axis_tracking=False, 
    ...
    pvgis_filename += '.json'
    pvgis_filepath = data_path('weather', pvgis_filename, repo_filepath=repo_filepath)
...
def get_random_location(country_code='DE', repo_filepath=''):
    ...
    overpass_filepath = data_path('buildings', overpass_filename, repo_filepath=repo_filepath)
...
def get_solar_gains(weather, bldg_params, albedo = 0.2):
    ...
    building_params: dict of dicts
        ...
        The structure for three example buildings is shown in i4b/data/buildings/.
...
def get_int_gains(time, profile_path, bldg_area = None):
    ...
    profile_path: string
        ...
        e. g.:  i4b/data/profiles/InternalGains/ResidentialDetached.csv
