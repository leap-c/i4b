# `production/` — the built corpus

This directory is **not tracked**: it is ~6.8 GB, and it is reproducible from `source-data/` by
the pipeline in `../scripts/`. Extract a released corpus here, or build one (see the repository
README), and the benchmark finds it without configuration — it is the default `dataset_dir`.
